...
// BLOX Module
const blox = new Blox();
await blox.initialize("mongodb://127.0.0.1:27017/","Blox", "pipeblock");
const client = await MongoClient.connect(
    "mongodb://127.0.0.1:27017/"
);
const dbMongo = client.db("Blox").collection("pipeblock"); // Will be added directly to the module in the future
//
...
async function index(network = 'main')
{
    ...
    try
    {
        try
        {
            ...
            const blockCheckM = await dbMongo.findOne({ type: 'bchk' });
            const blockCheckMV = blockCheckM && blockCheckM.value ? parseInt(blockCheckM.value) : null;

            if (block_check === block || blockCheckMV === block) {
        ...
        await dbMongo.updateOne({ type: 'mrk' }, { $set: { value: '' } }, { upsert: true });
        await dbMongo.updateOne({ type: 'bchk' }, { $set: { value: block } }, { upsert: true });
        await blox.load(block);
        ...
        for(let i = 0; i < tx.tx.length; i++)
        {
            try
            {
                ...
                let txHex = bitcoin.Transaction.fromHex(hex);
                let res = transactionToJson(txHex);
                let vsize = Math.round(txHex.virtualSize());
                ...
                for(let j = 0; j < res.vin.length; j++)
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            await blox.indexAction({ type: "t", txid: res.txid, vsize: vsize, tick: old_utxo.tick, id: old_utxo.id, value: Long.fromString(old_utxo.amt.toString())});
                ...
                )
                {
                    ...
                    {
                        case op_table.d:
                            await indexDeployment(block, blockhash, op_return_vout, tx.tx[i], res, decoded, vsize, network);
                            break;
                        case op_table.m:
                            await indexMint(block, blockhash, op_return_vout, tx.tx[i], res, decoded, vsize, network);
                            break;
                        case op_table.t:
                            await indexTransfer(block, blockhash, op_return_vout, tx.tx[i], res, decoded, vsize, network);
                            break;
                    }
                }
                else
                        ...
                        for(let j = 0; j < res.vout.length; j++)
                        {
                            ...
                            if( decoded[0] !== 'OP_RETURN' )
                            {
                                try
                                {
                                    ...
                                    const blox_data = {tick: pair[0].toLowerCase(), id: parseInt(pair[1])};
                                    ...
                                    try
                                    {
                                        ...
                                        await blox.indexAction({ type: "t", txid: res.txid, vsize: vsize, tick: blox_data.tick, id: blox_data.id, value: Long.fromString(spent_token_count[sig].toString())});
                                    }
                                    catch(e)
                                    {
        ...
        await blox.indexBlock();
        await dbMongo.updateOne({ type: 'blck' }, { $set: { value: block, lastPB: blox.pipeBlock} }, { upsert: true });
        await dbMongo.deleteOne({ type: 'mrk' });
 ...
 * @returns {Promise<void>}
 */
async function indexTransfer(block, blockhash, vout, tx, res, ops, vsize, network = 'main')
{
    ...
    if(utxos.length > 0)
    {
        ...
        for(let i = 0; i < utxos.length; i++)
        {
            ...
            try
            {
                ...
                await blox.indexAction({ type: "t", txid: tx.txid, vsize: vsize, tick: utxos[i].tick, id: utxos[i].id, value: Long.fromString(utxos[i].amt.toString()) });
 ...
 * @returns {Promise<void>}
 */
async function indexMint(block, blockhash, vout, tx, res, ops, vsize, network = 'main')
{
    if(ops.length !== 7) return;
    ...
    if(deployment !== null)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                await db.put(address_amt, amt.toString());
                await blox.indexAction({ type: "m", txid: tx.txid, vsize: vsize, tick: ticker, id: id, value: Long.fromBigInt(_mint), addr:to_address});
            }
            catch(e)
            {
 ...
 * @returns {Promise<void>}
 */
async function indexDeployment(block, blockhash, vout, tx, res, ops, vsize, network = 'main')
{
    try
    {
        ...
        catch(e)
        {
            ...
            if(mint_to_beneficiary)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        amt = BigInt(amt) + _mint;
                        await db.put(address_amt, amt.toString());
                        await blox.indexAction({ type: "m", addr: mint_to_beneficiary_to_address, txid: tx.txid, vsize: vsize, tick: ticker, id: id, value: Long.fromBigInt(_mint) });
            ...
            await blox.indexAction({ type: "d", txid: tx.txid, addr: _deployment.baddr, vsize: vsize, tick: _deployment.tick, id: _deployment.id, value: Long.fromString(_deployment.max), dec:_deployment.dec, art: _deployment.col ? true : false });
